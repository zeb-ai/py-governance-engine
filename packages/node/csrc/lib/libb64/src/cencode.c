...
/*
 * Calculate required length of buffer for encoded string when using encoder
 * state structure state_in for encoding and when having input data of size
 * plain_len
 *
 * Return encoded length, or 0 if encoded length + one additional null byte
 * would exceed range of size_t
 */
...
size_t base64_encode_length(size_t plain_len, base64_encodestate *state_in) {
  ...
  /*
   * Check for integer overflow. Takes into consideration that gcc may optimise
   * out naive integer overflow checks based on the C standard saying the result
  ...
  if (retval && cpl) {
    ...
    /* Make sure there's always room for one additional 0 byte on top of our
     * return value */
    if (numbreaks < retmax)
